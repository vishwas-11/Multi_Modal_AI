...
// Format: { mimeType: [Buffer signature(s)], offset: byte offset }
interface MagicSignature {
  bytes: number[];
  offset: number;
  mask?: number[]; // optional mask for wildcard bytes
}
 ...
 */
const readMagicBytes = async (filePath: string, count = 16): Promise<Buffer> => {
...
export const validateUploadedFiles = async (
  ...
  for (const file of files) {
    ...
    if (file.size > sizeLimit) {
      invalidFiles.push(
        `${file.originalname}: exceeds ${category} size limit (${Math.round(sizeLimit / 1024 / 1024)}MB)`
      );
      continue;
    }

    // Validate magic bytes
    const magicResult = await validateMagicBytes(file.path, file.mimetype);
